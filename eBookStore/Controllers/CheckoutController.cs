...
public class CheckoutController : Controller
{
  ...
  public async Task<IActionResult> Success()
  {
    ...
    var queueRepo = new QueueRepository(_connectionString);
    ...
    try
    {
      foreach (var item in cart.Items)
      {
        try
        {
          // Get current book details to check/update copies
          var book = bookRepo.getBookById(item.id);
          if (book == null)
          {
            throw new Exception($"Book {item.id} not found");
          }
          ...
          if (item.PurchaseType.ToLower() == "borrow")
          {
            ...
            {
              // Add to queue instead of borrowing
              await queueRepo.AddAsync(new BookRentQueueModel
              {
                bookId = item.id,
                userId = currentUserId.Value,
                createdAt = DateTime.UtcNow
              });
              repoLogger.LogInformation($"No copies available for book {item.id}, user {currentUserId.Value} added to queue");
              continue;
            }

            // Decrease available copies
            bookRepo.updateAmountOfCopies(item.id, book.book.amountOfCopies - 1);
