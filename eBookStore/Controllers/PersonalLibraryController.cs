...
public class PersonalLibraryController : Controller
{
  ...
  public async Task<IActionResult> Index()
  {
    try
    {
      ...
      // Check for expired books and handle returns
      var expiredBooks = borrowedBooks.Where(b => b.book.createdAt.AddDays(30) < DateTime.UtcNow).ToList();
      ...
      if (expiredBooks.Any())
      {
        foreach (var expiredBook in expiredBooks)
        {
          ...
          {
            // Return the book
            await _libraryRepo.ReturnBookAsync(userId.Value, expiredBook.book.id);

            // Remove from queue if user was in it
            await _queueRepo.DeleteAsync(userId.Value, expiredBook.book.id);

            // Increment the book's copy count using BookRepository
            var bookRepo = new BookRepository(_connectionString);
            var book = bookRepo.getBookById(expiredBook.book.id);
            if (book != null)
            {
              bookRepo.updateAmountOfCopies(expiredBook.book.id, book.book.amountOfCopies + 1);
            }

            // Check queue and notify next user if any
            var queue = await _queueRepo.GetQueue(expiredBook.book.id);
            if (queue.Any())
            {
              var nextInLine = queue.OrderBy(q => q.createdAt).First();
              // TODO: Add notification logic here
              _logger.LogInformation("Next user in queue for expired book {BookId}: {UserId}",
                  expiredBook.book.id, nextInLine.userId);
            }

            _logger.LogInformation("Automatically returned expired book {BookId} for user {UserId}",
                expiredBook.book.id, userId.Value);
          }
          catch (Exception ex)
          {
            _logger.LogError(ex, "Error processing expired book {BookId} for user {UserId}",
                expiredBook.book.id, userId.Value);
          }
        }
        ...
        {
          TempData["Warning"] = "One of your borrowed books has expired and has been automatically returned.";
        }
        else
        {
          TempData["Warning"] = $"{expiredBooks.Count} of your borrowed books have expired and have been automatically returned.";
        }

        // Refresh borrowed books list after processing returns
        borrowedBooks = (await _libraryRepo.GetBorrowedBooksAsync(userId.Value)).ToList();
      }
      ...
      if (TempData["Success"] == null && userBooks.Count == 0 && !borrowedBooks.Any())
      {
